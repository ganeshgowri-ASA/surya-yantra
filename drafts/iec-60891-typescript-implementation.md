...
| P2 — Multiplicative | αᵣᵉˡ, β, Rₛ, κ | Large irradiance translations, indoor flash |
| P3 — Bilinear | Two reference curves | Unknown module parameters |
| P4 — Extended parametric | αᵣᵉˡ, β, Rₛ, κ, Rₛʰ | Very low irradiance, leakage-dominated |
...
P2 produces slightly lower Pmpp because its multiplicative current model avoids the linear over-prediction that P1 exhibits near Voc.
...
- **GanitaSutra-v0 connection**: GanitaSutra’s PV toolbox (active sprint as of May 2026) will add a curve-fitting module that extracts αᵣᵉˡ and Rₛ from two measured curves automatically — making P3 the zero-configuration default correction path.
...
```
I₂ = I₁ · (1 + αᵣᵉˡ · ΔT) · (G₂/G₁) + (V₁ / R_sh) · (G₂/G₁ − 1)
V₂ = V₁ + β · ΔT − Rₛ · (I₂ − I₁) − κ · I₂ · ΔT
```
...
<!-- TODO: Add table of validation results against Fraunhofer ISE reference calculations -->
...
5. Surya Yantra source: `apps/web/lib/iec60891.ts`, `apps/web/__tests__/lib/iec60891.test.ts`.
