...
stale: false
stale_audit_note: >
  7 days old; active blocking issues #142, #152; substantial 9 kB outline —
  retained. New lint gaps: §5.3 figure alt-text missing (issue #170), 4 TODO
  citation DOIs refs 5–8 (issue #171), apps/web/lib/api-auth.ts referenced
  but absent from lib/ (issue #172).
---
...
- Planned: `requireAuth()` from `apps/web/lib/api-auth.ts`.
  **Note (2026-06-16 lint):** `apps/web/lib/api-auth.ts` does not yet exist in
  the repository — tracked as issue #172.
...
>
> **⚠️ Alt-text required when figure is added (WCAG 1.1.1, issue #170):**
> Suggested alt-text: "Dual-path diagram showing raw IV points flowing left-to-
> right from ESL-Solar 500 instrument via Socket.IO (streaming path) to
> LiveIVChart browser component, and via REST POST /api/corrections/p2
> (post-hoc correction path) to the STC report output."
...
6. *(TODO: IEC 60904-7:2019 citation — spectral mismatch — issue #171)*

7. *(TODO: Recharts performance benchmark citation — issue #171)*

8. *(TODO: India solar testing capacity reference — MNRE or IEA — issue #171)*
