...
*Published 2 May 2026 · Updated 9 May 2026 · Srishti PV Lab, Jamnagar*
...
- **[SolarLabX](https://github.com/ganeshgowri-ASA/SolarLabX)** — LIMS + QMS + audit trail for IEC-accredited labs (18 open issues, pushed 8 May 2026 — active LIMS sprint). Surya Yantra pushes corrected IV measurements directly into SolarLabX's test record database via the `/api/measurements` webhook.
- **[GanitaSutra-v0](https://github.com/ganeshgowri-ASA/GanitaSutra-v0)** — MATLAB-inspired computation platform (21 toolboxes, SimuFlow block diagrams). GanitaSutra's curve-fitting module (active sprint, **16 open issues** as of 8 May 2026) will automatically extract temperature coefficients and series resistance from two measured curves, removing the need for module datasheet parameters in P3/P4 corrections. **Targeted for Q2 2026.**
- **[Antaryami-OS](https://github.com/ganeshgowri-ASA/antaryami-os)** — Enterprise AI operating system (**42 open issues** as of 9 May 2026 — pushed today, +9 in 24 hours). Orchestrates multi-lab test scheduling and routes AI diagnostics queries to the optimal model (Claude Opus 4.7 for deep fault analysis, Haiku 4.5 for rapid triage). The minimum viable `pv-session-planner` webhook integration is **targeted for Q2 2026**.
- **[ShilpaSutra](https://github.com/ganeshgowri-ASA/ShilpaSutra)** — AI-powered text/multimodal to CAD & CFD platform (**37 open issues**, pushed today 9 May 2026 — active sprint). Will generate parametric CAD models of the test rack geometry and Kelvin harness routing from constraint descriptions, populating the missing `hardware/schematics/` directory (issue #13).

## Roadmap

- **Q2 2026** — STM32H7 MUX controller firmware published in `hardware/firmware/` (issue #2)
...
- **Q2 2026** — Antaryami-OS `pv-session-planner` webhook integration (irradiance-aware test queue scheduling)
- **Q2 2026** — ShilpaSutra parametric CAD for rack geometry and Kelvin harness routing (issue #13)
- **Q3 2026** — IEC 61853-3 energy rating pipeline (G–T matrix → annual energy yield)
- **Q3 2026** — Full Antaryami-OS skill orchestration: `pv-fault-router` + `multi-lab-coordinator`
